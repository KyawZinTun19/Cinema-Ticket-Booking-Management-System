...
<head>
  <meta charset="UTF-8">
  <title>Searched Movie Table</title>
...
<div class="container">
	<div class="labelContainer"><label class="label">Searched Movie Table</label></div>
	<div class="top">
	...
	<input type="text" name="name" placeholder="Genre" id="genreInput">
		<button class="SearchButton" onclick="search()">Search</button>
		<button class="AddButton" onclick="location.href='/CinemaTicketBooking/setupAddMovie'">Add</button>
...
function search() {
	  ...
	  var searchParams = [];
	  if(id!=="" && name !== ""&& genre !== ""){
		  searchParams.push("MovieAll/" + id+"/"+name+"/"+genre);
...
<a href="/CinemaTicketBooking/deleteMovie/${movielist.movieID}" class="DeleteButton" onclick="return confirmDelete(event)" style="text-decoration:none;display: inline-block;">Delete</a>
        ...
        <c:forEach items = "${deletedmovielist}" var="deletedmovielist">
             <tr>
				...
				<td style="text-align: center;">
				<button onclick="location.href='/CinemaTicketBooking/setupDetailMovie/${movielist.movieID}'" class="DetailsButtonDeleted" >Details</button>
			</tr>
        </c:forEach>
